...
`).trim()}function Ko(l,o){return JSON.stringify({version:1,source:"mAI Coder browser capture",exportedAt:new Date().toISOString(),requestCount:l.length,scope:o,requests:l},null,2)}function Wo(l){return JSON.stringify({log:{version:"1.2",creator:{name:"mAI Coder browser capture",version:"1.0"},pages:[],entries:l.map(o=>{const d=o.requestBody??"",b=o.responseBody??"",w=o.contentType??gr(o.responseHeaders,"content-type"),h=gr(o.requestHeaders,"content-type"),E=o.durationMs??0;return{startedDateTime:new Date(o.startedAt||Date.now()).toISOString(),time:E,request:{method:o.method,url:o.url,httpVersion:"HTTP/1.1",cookies:[],headers:Ut(o.requestHeaders),queryString:qo(o.url),headersSize:-1,bodySize:d.length,...d?{postData:{mimeType:h,text:d}}:{}},response:{status:o.status??0,statusText:o.errorText??"",httpVersion:"HTTP/1.1",cookies:[],headers:Ut(o.responseHeaders),content:{size:b.length,mimeType:w,...b?{text:b}:{}},redirectURL:gr(o.responseHeaders,"location"),headersSize:-1,bodySize:b.length},cache:{},timings:{blocked:-1,dns:-1,connect:-1,send:0,wait:E,receive:0,ssl:-1},...o.errorText?{comment:o.errorText}:{}}})}},null,2)}function $t(l,o,d){const b=new Blob([d],{type:o}),w=URL.createObjectURL(b),h=document.createElement("a");h.href=w,h.download=l,h.rel="noopener",document.body.appendChild(h),h.click(),h.remove(),window.setTimeout(()=>{URL.revokeObjectURL(w)},1e3)}function Jo(l){return`async-browser-capture-${new Date().toISOString().replace(/[:.]/g,"-")}.${l}`}function Gt(l,o=Yt){const d=Number.isFinite(l)?l:Jt;return Math.min(Math.max(Math.round(d),Vt),o)}async function ye(l,o){if(l)try{await l.invoke("browser:commandResult",o)}catch{}}let zt=0;function wr(l=Wt){return zt+=1,{id:`browser-tab-${Date.now().toString(36)}-${zt}`,requestedUrl:l,currentUrl:l,draftUrl:l,pageTitle:"",isLoading:!0,canGoBack:!1,canGoForward:!1,loadError:null}}const Vo=n.memo(function({tab:o,partition:d,userAgent:b,fingerprintScript:w,active:h,hookEnabled:E,hookScript:F,onHookEvents:t,onStorageSnapshot:c,t:C,onNavigate:U,onTitle:ie,onLoading:ve,onFailLoad:O,onRegisterWebview:T}){const S=n.useRef(null),V=n.useRef(null);V.current=w;const Y=n.useRef(F);Y.current=F;const G=n.useRef(Lo()),He=n.useRef(E);He.current=E;const de=n.useRef(t);de.current=t;const De=n.useRef(c);De.current=c;const q=n.useRef(o.id),[le,We]=n.useState(null);q.current=o.id;const _=n.useCallback(()=>{const m=S.current,N=m?.parentElement;if(!m||!(N instanceof HTMLElement))return;const P=Math.max(1,Math.round(N.clientWidth)),A=Math.max(1,Math.round(N.clientHeight));We(g=>g&&g.width===P&&g.height===A?g:{width:P,height:A})},[]),xe=n.useCallback(m=>{S.current=m;try{T(q.current,m)}catch(N){console.error("[BrowserTab] error in onRegisterWebview:",N)}},[T]);n.useEffect(()=>{const m=S.current;if(!m)return;const N=()=>{try{return{canGoBack:!!m.canGoBack?.(),canGoForward:!!m.canGoForward?.()}}catch{return{canGoBack:!1,canGoForward:!1}}},P=()=>{ve(q.current,!0);const W=V.current;W&&m.executeJavaScript(W,!1).catch(()=>{}),m.executeJavaScript(G.current,!1).catch(()=>{})},A=()=>{ve(q.current,!1,te(m))},g=W=>{const v=W;if(v.isMainFrame===!1)return;const ae=String(v.url??te(m)??"").trim(),{canGoBack:oe,canGoForward:vr}=N();U(q.current,{currentUrl:ae,canGoBack:oe,canGoForward:vr})},R=W=>{ie(q.current,String(W.title??"").trim())},X=()=>{const{canGoBack:W,canGoForward:v}=N();U(q.current,{currentUrl:te(m),canGoBack:W,canGoForward:v});const ae=V.current;ae&&m.executeJavaScript(ae,!1).catch(()=>{}),m.executeJavaScript(G.current,!1).catch(()=>{}),He.current&&Y.current&&m.executeJavaScript(Y.current,!1).catch(()=>{})},ce=W=>{const v=W;if(v.isMainFrame===!1||v.errorCode===-3)return;const ae=String(v.validatedURL??te(m)??"").trim();O(q.current,{message:String(v.errorDescription??C("app.browserLoadFailed")),url:ae})};return m.addEventListener("dom-ready",X),m.addEventListener("did-start-loading",P),m.addEventListener("did-stop-loading",A),m.addEventListener("did-navigate",g),m.addEventListener("did-navigate-in-page",g),m.addEventListener("page-title-updated",R),m.addEventListener("did-fail-load",ce),()=>{m.removeEventListener("dom-ready",X),m.removeEventListener("did-start-loading",P),m.removeEventListener("did-stop-loading",A),m.removeEventListener("did-navigate",g),m.removeEventListener("did-navigate-in-page",g),m.removeEventListener("page-title-updated",R),m.removeEventListener("did-fail-load",ce)}},[d,ve,U,ie,O]),n.useEffect(()=>{if(!E)return;let m=!1;const N=async()=>{const A=S.current;if(!(!A||m))try{const g=await A.executeJavaScript("(function(){ try { return window.__asyncDrainHooks ? window.__asyncDrainHooks() : []; } catch(_) { return []; } })()",!1);if(m)return;Array.isArray(g)&&g.length>0&&de.current(q.current,g)}catch{}},P=window.setInterval(N,1500);return N(),()=>{m=!0,window.clearInterval(P)}},[E,o.id]),n.useEffect(()=>{if(!E)return;let m=!1;const N=`(function(){
			try {
