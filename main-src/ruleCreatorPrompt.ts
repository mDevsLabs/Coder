...
export function buildRuleCreatorSystemAppend(
	...
	const globHint =
		ruleScope === 'glob'
			? lang === 'en'
				? `The user chose **glob-scoped** rules. Target glob (relative to workspace): \`${(globPattern ?? '').trim() || '(user should refine)'}\`. Explain how it maps to mAI Coder **Settings → Agent → Rules** with scope "glob".`
				: `用户选择 **Glob 范围** 规则。目标 glob（相对工作区）：\`${(globPattern ?? '').trim() || '（请与用户确认）'}\`。说明如何对应 mAI Coder **设置 → Agent → Rules** 中的「Glob」范围与填写方式。`
			: '';

	const wizardChoiceBlock =
		ruleScope === 'always'
			? lang === 'en'
				? '**Already chosen in mAI Coder wizard:** always attach (every turn). In question (2) below, confirm they still want always-on behavior or note if they want to switch to file-based globs.'
				: '**用户在 mAI Coder 向导中已选：**「始终附加」（每轮对话都带上）。下方第 2 点可简化为确认是否仍要全局生效；若用户想改成按文件匹配，再引导补充 globs。'
			: ruleScope === 'glob'
				? lang === 'en'
					? `**Already chosen in mAI Coder wizard:** glob-scoped. Preset glob: \`${(globPattern ?? '').trim() || '(ask user)'}\`. In question (2), ask them to confirm or list additional patterns (comma or newline).`
					: `**用户在 mAI Coder 向导中已选：**「路径 Glob」。预设 glob：\`${(globPattern ?? '').trim() || '（请与用户确认）'}\`。第 2 点请对方确认该模式或补充多条（逗号或分行）。`
				: lang === 'en'
					? '**Already chosen in mAI Coder wizard:** manual @ only. In question (2), ask how they want the rule named for @-mentions and confirm it will not auto-attach.'
					: '**用户在 mAI Coder 向导中已选：**「手动 @ 触发」。第 2 点说明：仅在 @ 引用时注入；请约定规则展示名 / 引用 id，并确认不会自动全局附带。';

	const scopeBlock =
		ruleScope === 'always'
			? lang === 'en'
				? '**Scope: Always attach.** Persist as `.mai/rules/<name>.mdc` with `alwaysApply: true` (and typically no `globs`, or empty).'
				: '**范围：始终附加。** 落盘为 `.mai/rules/<name>.mdc`，`alwaysApply: true`（通常不写或留空 `globs`）。'
			: ruleScope === 'glob'
				? globHint
				: lang === 'en'
...
要用 /create-rule 帮你写好一条可用的规则（将写入工作区 **\`.mai/rules/\`**），需要先确定几件事（你按条回复即可）。
...
收到「目的 + 作用范围 +（若有）globs」后，在已打开工作区时用 \`Write\` 在 \`.mai/rules/\` 下新增对应的 \`.mdc\` 文件（含正确的 YAML frontmatter，内容尽量简洁、可执行，并带简短正反例）。若目录不存在请先创建再写入。落盘后一两句话说明路径即可。`;
...
To write a solid mAI Coder workspace rule (\`.mdc\` under \`.mai/rules/\`) via /create-rule, a few quick answers help (reply point-by-point):
...
**After answers:** Tell the user you will add the \`.mdc\` under \`.mai/rules/\` with correct YAML frontmatter, concise actionable body, and short good/bad examples—then actually do it with \`Write\` when the workspace is open (create the directory if needed); end with a brief path note.`;

	const toolBlock =
		lang === 'en'
			? `**Execution mode:** This turn runs in **Agent** with tools \`Write\` and \`Edit\` on the open workspace.
- If a workspace folder is open (\`Workspace root\` below is not "(none)"), you **must** create or update rules under **\`.mai/rules/\`** (e.g. \`.mai/rules/my-rule.mdc\`). That is the canonical mAI Coder location—do not use \`.cursor/rules/\` unless the user explicitly asks. Do **not** make "copy-paste this into Settings" your primary answer—write the files with tools, then briefly tell the user what you created.
- If **no** workspace is open, you cannot use write tools; say so clearly and give the shortest path: open a folder, or paste into mAI Coder **Settings → Agent → Rules**—but do not pretend files were written.
...
- 若已打开工作区（下方「工作区根目录」不是「（无）」），你**必须**把规则写入 **\`.mai/rules/\`**（例如 \`.mai/rules/my-rule.mdc\`）。这是 mAI Coder 约定目录；**不要**默认写到 \`.cursor/rules/\`，除非用户明确要求。**禁止**把「请用户整段复制到设置里」当作主要交付；应优先落盘，再用一两句话说明写入了哪些路径。
- 若**未**打开工作区，无法写盘，需明确说明，并给出最短路径：先打开文件夹再重试，或手动粘贴到 mAI Coder **设置 → Agent → Rules**；不要假装已写入文件。
- 落盘后可用简短文字说明当前**范围**（${ruleScope}）及 Glob（若有）。`;

	const mdcShape =
		lang === 'en'
			? `**\`.mdc\` under \`.mai/rules/\` (reference, Cursor-compatible frontmatter):**
\`\`\`yaml
---
...
---
\`\`\`
Match \`alwaysApply\` / \`globs\` to the user's answers and the wizard scope (${ruleScope}). Save as e.g. \`.mai/rules/<slug>.mdc\`.`
			: `**\`.mai/rules/\` 下的 \`.mdc\`（参考，frontmatter 与 Cursor 习惯兼容）：**
\`\`\`yaml
---
...
---
\`\`\`
frontmatter 须与用户答复及向导范围（${ruleScope}）一致；文件路径形如 \`.mai/rules/<slug>.mdc\`。`;

	const core =
		lang === 'en'
			? `You are the **Rule Author** wizard for mAI Coder (project rules as \`.mdc\` under \`.mai/rules/\`). The user's message is after the scope tag.

**Supersedes generic create-rule skills:** If another block in context mentions \`.cursor/rules/\`, **this wizard wins** — on-disk path is always \`.mai/rules/\` when a workspace is open.
...
${scopeBlock}`
			: `你是 mAI Coder 的 **Rule 编写向导**（在工作区 \`.mai/rules/\` 下生成 \`.mdc\`，frontmatter 可与 Cursor 习惯兼容）。用户消息在范围标签之后。

**优先于通用 create-rule Skill：** 若上下文里另有说明写 \`.cursor/rules/\`，**以本向导为准**——有工作区时落盘目录一律为 \`.mai/rules/\`。
...
${scopeBlock}`;
	...
	return `### mAI Coder · Rule Creator（内置）\n\n${core}`;
}
