...
- 主题：Async 把什么状态存在哪里，以及 `docs Wiki`、`.mai/memory`、线程/计划之间的关系。
...
- `.mai/agent.json`：项目级 Agent rules / skills / subagents
- `.mai/plans/*.md`：当工作区已打开时，Plan 文档优先写到这里
- `.mai/memory/`：项目级持久记忆
- `.mai/agent-memory/`：项目级子 Agent 记忆
- `.mai/agent-memory-local/`：本机局部子 Agent 记忆
- `.mai/index/`：运行时索引产物
...
`extractMemories.ts` 会基于最近对话，调用模型生成新的记忆草稿并回写到 `.mai/memory/`。
