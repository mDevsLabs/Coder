...
export const messagesFr: Record<string, string> = {
	...
	'settings.general.identityPreset.async': 'Par défaut mAI Coder',
	...
	'settings.general.identityPresetAsyncHint': 'Utilise les valeurs par défaut mAI Coder : le User-Agent style mAI Coder, X-Async-Session-Id, les métadonnées mAI Coder et le préfixe d’invite système mAI Coder.',
	...
	'agentSettings.leadCursor': 'Règles, skills et sous-agents pour l’agent. Filtrez par portée ci-dessus. Les dossiers d’espace de travail pour `.cursor`, `.claude` et `.async` sont fusionnés automatiquement.',
	...
	'agentBehavior.memoryExtractionDesc': 'Limite l\'extraction en arrière-plan vers `.async/memory`. Désactivez pour arrêter la mise en file d\'attente des extractions.',
	...
	'skillCreator.scopeProjectHint': 'Espace de travail uniquement, ex. `.async/skills/<slug>/SKILL.md`.',
	...
	'slashCmd.createSkillDesc': 'Créer un Skill : sélecteur de portée, puis mode Agent — privilégiez l’écriture de `.async/skills/.../SKILL.md` avec les outils.',
	'slashCmd.createRuleDesc': 'Créer une Règle : après le choix de la portée, passe en mode Agent pour que l’assistant écrive `.async/rules/*.mdc` (pas seulement un collage dans le chat).',
	'slashCmd.createSubagentDesc': 'Créer un Sous-agent : choisissez la portée, puis mode Agent — préférez mettre à jour `.async/agent.json` (etc.) avec les outils, avec mémoire persistante facultative.',
	...
	'ruleWizard.desc': 'Choisissez comment la règle est injectée ; l\'envoi bascule vers Agent afin que l\'assistant puisse créer/mettre à jour les fichiers `.mdc` sous `.async/rules/` lorsqu\'un espace de travail est ouvert.',
	...
	'settings.indexing.lead': 'Inspectez la couche mémoire du projet et gérez les fichiers que mAI Coder utilise pour stocker les connaissances durables du projet sous `.async/memory/`.',
	...
	'settings.indexing.memoryLayoutDesc': 'La mémoire du projet se trouve sous `.async/memory/`. Gardez `MEMORY.md` court et faites des liens vers des fichiers thématiques au lieu de tout déverser dans un seul fichier.',
